...
/**
 * Instructions given to Gemini. We constrain the model hard so the output is a
 * single, self-contained, render-safe Manim scene.
 */
...
- The code must run with no arguments via: manim -q<quality> scene.py <ClassName>
...
/** JSON schema handed to Gemini's responseSchema for reliable structured output. */
...
/**
 * Tokens that must never appear in generated code. The generated Python is
 * arbitrary code executed by the render worker, so we do a defensive static
 * scan before ever rendering it. This is a guardrail, not a real sandbox — the
 * worker also runs isolated (see render-worker/README).
 */
const DENYLIST: { pattern: RegExp; reason: string }[] = [
  ...
  { pattern: /\bgetattr\s*\(\s*__/, reason: "dunder getattr" },
];
...
/** Static safety + sanity check on the generated scene. */
export function validateManimCode(scene: GeneratedScene): Validated {
  ...
  const classRe = new RegExp(`class\\s+${sceneName}\\s*\\(`);
